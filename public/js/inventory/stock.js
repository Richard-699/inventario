        ...
        ajax: {
            ...
            dataSrc: function (json) {
                const infoConteoStock = (json.informacionSAP || []).map((item) => ({
                    ...
                    id_part_number_informacion_sap_mb52: item.id_part_number_informacion_sap_mb52,
                ...
                if (json.informacionSAP && json.informacionSAP.length > 0) {
                    const item = json.informacionSAP.find((data) => data.id_almacen_informacion_sap_mb52 === id_almacen);
                    let almacen = null;
                    let partnumber = null;
                    ...
                    const almacenHTML = `
                        ...
                        <hr class="mt-4">
                    `;
                    const contenedorTitulo = document.querySelector(".d-flex.align-items-center.justify-content-between.mb-4.border-bottom.pb-2");
                ...
                // 1. Crear un objeto para almacenar localizaciones únicas
                const localizacionesUnicas = {};
                // 2. Iterar sobre las localizaciones y añadir solo las únicas al objeto
                json.localizaciones.forEach(loc => {
                    localizacionesUnicas[loc.id_localizacion] = loc;
                });
                // 3. Convertir el objeto de localizaciones únicas de vuelta a un array
                const localizacionesArrayUnicas = Object.values(localizacionesUnicas);

                // 4. Mapear el array de localizaciones únicas para añadir el stock
                return localizacionesArrayUnicas.map((loc) => {
                    const stockItem = json.stock.find((s) => s.id_localizacion_stock == loc.id_localizacion);
                    return {
                        ...loc,
...
async function conteo(btn, id) {
    ...
    const jsonFinal = {
        ...jsonConteoStock,
        informacionSAP: infoConLocalizacion
    };
    ...
    try {
        ...
        const params = new URLSearchParams({
            ...
            id_part_number_informacion_sap_mb52: d.id_part_number_informacion_sap_mb52 ?? "",
